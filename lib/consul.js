...
Storage.prototype.register_self = function ( ) {
  ...
  var search = {
    cluster: this.service || 'cluster', // .service.consul',
  ...
  var insert = {
    cluster: {
      ...
      checks: [ {
        ...
        ttl: '36s',
        interval: '16s',
        http: url.format({protocol: 'http', hostname: address.host, port: address.port,  pathname: '/stats/active'}),
  ...
  var insertion = insert['cluster'];

  function do_insert ( ) {
    consul.agent.service.register(insertion, function (err, resp) {
      if (err) {
        console.log("TROUBLE CONNECTING TO CONSUL", err, resp);
      }
      console.log("CREATED BACKBONE CLUSTER", insertion);
    });
  }
  var search = {id: insertion.id, service: insertion.name };
  consul.catalog.service.nodes(search, function (err, resp) {
...
Storage.prototype.handleSiteUp = function (worker, address) {
  ...
  // var port = url.parse(this.url).port;
  // var host = url.parse(this.url).hostname;
  var multienv = {host: this.host, port: this.port};
  var host = this.host;
  ...
  var backend_tag = this.service;
  var insert = {
		name: 'backends',
		address: host,
		port: port,
		id: id,
    tags: [ this.config.backends_id, name, 'tenant', backend_tag ],
		checks: [ {
  ...
  var search = {id: id, tag: name, service: insert.name };
  consul.catalog.service.nodes(search, function (err, resp) {
    console.log("consul precheck", search, err, resp ? resp.length : null);
    var found = resp ? resp.pop( ) : null;
    if (found && found.ServicePort == insert.port && found.ServiceAddress == insert.address) {
      console.log("skipping duplicate", id);
      return;
    } else {
      if (found && found.ID) { id = found.ID; }
      // consul.agent.service.deregister(id, function (err, resp) { });
        if (found) { console.log("found stale duplicate BACKEND SITE", err, found, insert, resp); }
        consul.agent.service.register(insert, function (err, resp) { console.log("CREATED BACKEND SITE", id); });
    }
  });
}
