...
function createNightscoutTenantRoutes(kc, namespace, cfg) {
  ...
  function templatePVC(storageAccountId, cfg, options = {}) {
    ...
    const storageSize = options.storageSize || cfg.default?.storageSize || '10Gi';
   ...
   * Name: {tenantId}-nightscout or custom name
   */
  function templateConfigMap(storageAccountId, tenantId, cfg, options = {}) {
    const configMapName = options.configMapName || `${tenantId}-nightscout`;
  ...
  function handleCreateOrUpdateConfigMap(req, res, next) {
    ...
    const configMapName = req.body?.configMapName || `${tenantId}-nightscout`;
