...
## Recent Changes
- **2025-11-24**: Migrated from spec.selector-based ConfigMap discovery to explicit spec.configMapRef pattern for Gen5 NightscoutTenant CR. Replaced detectComputeActivation with ensureConfigMap supporting two patterns: (A) explicit spec.configMapRef for provisioner-managed ConfigMaps (migration/adoption with preserved name/namespace), (B) default ConfigMap creation when no reference provided (uses tenantId as name with Nightscout runtime defaults). Created renderDefaultConfigMap helper with sensible userdata defaults (DISPLAY_UNITS, CUSTOM_TITLE, SHOW_RAWBG, etc.). Compute now always enabled - ConfigMap guaranteed to exist via adoption or default creation. Pattern simplifies provisioner integration and ensures consistent ConfigMap presence. Architect-validated as production-ready.
