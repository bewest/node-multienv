...
- **Federation Migration (Proposed)**: Alternative to ephemeral storage—migrate from Digital Ocean Kubernetes (DOKS, ~28 volume limit) to GKE (128 volume limit) using KubeFed and Velero. Preserves PVC-based storage model with zero data loss. Includes Consul WAN federation for cross-cluster service discovery, DNS TTL playbook for zero-downtime cutover, and comprehensive rollback procedures. Cross-cluster migration jobs run in DOKS (source) to access legacy shared MongoDB on private IPs (10.0.50.x), targeting GKE Gen5 pods via Consul WAN DNS (recommended) or VPN with direct pod IP (opt-in). See `docs/FEDERATION-MIGRATION-PROPOSAL.md`.
