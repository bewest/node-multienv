...
- **Two-Phase Migration Architecture**: Gen3→Gen4 migration uses sequential phases for data and userdata separation. Phase 1 (Storage): Storage-Credentials Decorator watches ComputeInstance, migrates MongoDB data via migration Job, sets `nightscout.io/migration-completed` annotation on ComputeInstance. Phase 2 (Userdata): Instance-Userdata Decorator watches ConfigMap, discovers related ComputeInstance via customize hook, waits for storage completion annotation, archives complete ConfigMap (WITH `data.mongo` URI) to archive namespace for rollback capability, strips `data.mongo` URI from active ConfigMap (Gen4 uses Secret instead), sets `nightscout.io/userdata-migration-completed` annotation on ConfigMap. Archived ConfigMap enables safe rollback; active ConfigMap becomes Compute Composite child (lifecycle-managed, deleted on factory reset). Decorators are loosely coupled via annotation signaling.
