...
- **Federation Migration (Proposed)**: Alternative to ephemeral storage—migrate from AWS (28 volume limit) to GKE (128 volume limit) using KubeFed and Velero. Preserves PVC-based storage model with zero data loss. Includes Consul WAN federation for cross-cluster service discovery, DNS TTL playbook for zero-downtime cutover, and comprehensive rollback procedures. See `docs/FEDERATION-MIGRATION-PROPOSAL.md`.
