...
- **URI-Based Migration**: Migration Jobs use symmetric MongoDB URI format for both source and target (MIGRATION_SOURCE_URI and MIGRATION_TARGET_URI). The migrate-database.sh script expects complete connection URIs including authentication and database pathname segments, simplifying the interface and enabling retry reliability with embedded credentials.
