...
- **Federation Migration (Proposed)**: Alternative to ephemeral storage—migrate from Digital Ocean Kubernetes (DOKS, 7 volume limit per node) to GKE (128 volume limit, aligns with 110 pod/node default). Gen3b uses shared MongoDB on droplets (no PVCs), Gen5 on GKE uses per-tenant dedicated PVCs. Migration is pure data migration via mongodump/mongorestore. Cross-cluster migration jobs run in DOKS to access legacy shared MongoDB on private IPs (10.0.50.x), targeting GKE Gen5 pods via Consul WAN DNS. Proves multi-cluster federation patterns for future capacity expansion. See `docs/FEDERATION-MIGRATION-PROPOSAL.md`.
